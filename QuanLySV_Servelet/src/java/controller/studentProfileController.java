...
public class studentProfileController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (staffId == 1) {
            System.out.println("admin: " + staffId);
            String studentId = request.getParameter("id");
            Integer studentIdInteger = Integer.valueOf(studentId);
            System.out.println("StaffID: " + staffId);
            StaffDAO staff = new StaffDAO();
            Staff student = staff.selectById(studentIdInteger);
            request.setAttribute("staff", student);
            RequestDispatcher dispatcher = request.getRequestDispatcher("studentProfileAdmin.jsp");
            dispatcher.forward(request, response);
        } else {
            StaffDAO staff = new StaffDAO();
            Staff student = staff.selectById(staffId);
            System.out.println("student: " + staffId);
            request.setAttribute("staff", student);
